...
build_iOS()
{
	...
	export CFLAGS="-fembed-bitcode -isysroot $SDKROOT -arch ${ARCH} -miphoneos-version-min=6.0"
	export LDFLAGS="-arch ${ARCH} -isysroot $SDKROOT"

	if [ $ARCH == "i386" ] || [ $ARCH == "x86_64" ];
	then
		./configure --prefix=$XZPATH/build/iOS/$ARCH --host=i686-apple-darwin11 --enable-static 
	else
		./configure --prefix=$XZPATH/build/iOS/$ARCH --host=arm-apple-darwin --enable-static
	fi
